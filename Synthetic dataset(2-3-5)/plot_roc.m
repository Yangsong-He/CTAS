...
%         fill(curve(:,1),curve(:,2),  fill_color,'FaceAlpha',0.5);
        ...
        hold on; plot(curve_new(:,1),curve_new(:,2),  '-o','color',[0.07,0.62,1.00], 'LineWidth', 3,'MarkerSize',5);  % 
...
%         hold on; plot(curve(opt,1),curve(opt,2),  'xr', 'MarkerSize', 12);
        ...
        hold off; axis square; grid on; xlabel('FPR'); ylabel('TPR');
        set(gca,'FontSize',15)
        axis([0 1 0 1])
        title(['AUC = ' num2str(param.AROC)],'FontWeight', 'bold');
      end
