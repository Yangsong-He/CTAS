function [Para_Post_pdf,S,AM,S_new]=ODE_BayesianLasso_new1(Data_ordered,Data_tf,TimeSampled)
% Input:
% Data_ordered -- smoothed pseudo-progression RNA-binding protein expression data
% Data_tf -- smoothed pseudo-progression TF expression data
% TimeSampled -- Time points associate with pseudo-progression gene expression data
% Output:
% Para_Post_pdf -- posterior distribution over the coefficients in the Equation(2)
% S --  a matrix saving the presence probability. 
% AM -- Adjacent matrix of the inferred network 
%i.e., (c_lk) for the regulatory coefficient from RNA-binding protein k to RNA-binding protein l. 
%(e_lp)  for the regulatory coefficient from TF p to RNA-binding protein l.
%% data
Time=TimeSampled;
...
L=size(v,1);
M=size(Time,2);
%% Calculate Post_pdf for each RNA-binding protein's parameters
Para_Post_pdf=cell(size(x,1));
for ind_gene=1:size(x,1)
y_output=y(ind_gene,:);
x_input=[x',v',ones(size(x,2),1)].*x(ind_gene,:)';
x_input(:,ind_gene)=[];    
%% Bayesian Lasso
p=N+L;
