...
for NoiseLevel=grid_noiselevel  
    NoiseLevel
    for rep=1:100  %% repeat random simulation 200 times
        rep
    Synthetic_data;
...
xlabel('Noise level (%)','FontSize',20);
figure,
errorbar(grid_noiselevel,mean(coeff_Spearman,2),std(coeff_Spearman,0,2),'o-.','color',[0.5 0.5 0.5],'LineWidth',1,'MarkerSize',8,'MarkerFaceColor',[0.1 0.3 1]);title('Spearman correlation for progression inference'); axis([-1 31 0.5 1]); % axis([-1 31  0 max(mean(S_distance,2)+std(S_distance,0,2))])
set(gca,'FontSize',15)
xlabel('Noise level (%)','FontSize',20);